...
int *video::write_char_to_frame(AVFrame *fr, bitset<8> bs) {
    spdlog::trace("Writing char '{}' | Bitset '{}' at pos ({}, {})", subtitle::bin_to_char(bs), bs.to_string(),
        ...
        for (int block_y = 0; block_y < block_size; ++block_y) {
            for (int block_x = 0; block_x < block_size; ++block_x) {
                ...
                if (_write_x > fr->width - 1) {
                    spdlog::trace("Moving to new block line");
                    auto _write_y = this->write_y + block_size;

                    if (_write_y > fr->height - 1) {
                        //Handle reaching bottom of frame
                    }
    ...
    spdlog::trace("'{}' vs {} | ({}, {})", this->read_char_from_frame(fr), subtitle::bin_to_char(bs), this->write_x,
               this->write_y);
    return nullptr;
}
...
char video::read_char_from_frame(AVFrame *fr) {
    spdlog::trace("Reading char from frame");
    ...
    for (size_t iter = 0; iter < 8; ++iter) {
        ...
        spdlog::trace("Total: {} Mean: {} Read x: {}", total, mean, read_x);
        this->read_x = this->read_x + block_size;
    }
    spdlog::trace("{} vs '{}' | ({}, {})", bs.to_string(), subtitle::bin_to_char(bs), this->read_x, this->read_y);
...
int video::perform_steg_frame(AVFrame *fr) {
    ...
    while (true) {
        if (write_line_bs->empty()) {
            ...
            if (line_length == -1) {
                spdlog::debug("No more data left in the steganographic file");
                return 0;
            } else if (line_length > length || this->write_y >= fr->height) {
                spdlog::debug("No more data left in the frame");
                return 1;
            }
        }
...
int video::generate_frame_headers(AVFrame *fr) {
    ...
    spdlog::debug("Length of header bits = {}", length_of_header_bits);
    //Bits per frame considering header size
    bits_per_frame = bits_per_frame - length_of_header_bits;
    spdlog::debug("Bits per frame = {}", bits_per_frame);

    int _no_of_frames = (int) ceil((double) no_of_bits / (double) bits_per_frame);
    spdlog::debug("Characters: {}\t No of bits: {}", this->subtitle_file->size, no_of_bits);
    spdlog::debug("Total frames: {}", _no_of_frames);
    for (int iter = 0; iter < _no_of_frames; ++iter) {
        ...
        spdlog::trace("Data in frame: {}", data_in_frame);
        frame_header fh(data_in_frame, filename, _no_of_frames, iter);
        spdlog::trace("Storing header: {}", fh.to_string());
        headers->push_back(fh);
        no_of_bits = no_of_bits - bits_per_frame;
    }

    spdlog::debug(headers->size());
    spdlog::debug(headers->front().to_string());
    return 0;
}

void video::write_steg_header(AVFrame *fr, frame_header h) {
    string h_string = h.to_string();
    spdlog::debug("Writing frame header: '{}' to frame {}", h_string, h.frame_number);
...
frame_header *video::read_steg_header(AVFrame *fr) {
    spdlog::debug("Reading steg_header from a frame");
    string found_header;
    ...
    while (true) {
        if (found_header.empty()) {
            ...
            if (regex_match(found_header, frame_header::header_regex)) {
                ...
                spdlog::debug("Found frame_header '{}' should be |L|", found_header);
                if(found_header != "|L|"){
                    return nullptr;
                }
            }
    ...
    spdlog::debug("Found header: {}", found_header);
...
int video::end(int retu) {
    ...
    if (retu < 0) {
        spdlog::error("Error occurred: {}", av_err2str(retu));
    }
...
int video::open_input_file() {
    ...
    if (retu < 0) {
        spdlog::error("Cannot find stream information");
        return retu;
    }
    ...
    for (unsigned int iter = 0; iter < input_format_context->nb_streams; iter++) {
        ...
        if (!dec) {
            spdlog::error("Failed to find decoder for stream #{}", iter);
        ...
        if (!codec_ctx) {
            spdlog::error("Failed to allocate the decoder context for stream #{}", iter);
            return AVERROR(ENOMEM);
        }
        ...
        if (retu < 0) {
            spdlog::error("Failed to copy decoder parameters to input decoder context for stream #{}", iter);
            return retu;
        }
        ...
        if (codec_ctx->codec_type == AVMEDIA_TYPE_VIDEO || codec_ctx->codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            if (retu < 0) {
                spdlog::error("Failed to open decoder for stream #{}", iter);
                return retu;
            }
        }
...
int video::open_output_file() {
    ...
    for (iter = 0; iter < input_format_context->nb_streams; iter++) {
        ...
        if (decoder_context->codec_type == AVMEDIA_TYPE_VIDEO || decoder_context->codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            if (!encoder) {
                spdlog::error("Necessary encoder not found");
            ...
            if (retu < 0) {
                spdlog::error("Cannot open video encoder for stream #{}", iter);
                return retu;
            }
            retu = avcodec_parameters_from_context(out_stream->codecpar, encoder_context);
            if (retu < 0) {
                spdlog::error("Failed to copy encoder parameters to output stream #{}", iter);
                return retu;
            }
            ...
            spdlog::error("Elementary stream #{} is of unknown type, cannot proceed", iter);
            return AVERROR_INVALIDDATA;
        } else {
            /* if this stream must be remuxed */
            retu = avcodec_parameters_copy(out_stream->codecpar, in_stream->codecpar);
            if (retu < 0) {
                spdlog::error("Copying parameters for stream #{} failed", iter);
                return retu;
            }
    ...
    if (!(output_format_context->oformat->flags & AVFMT_NOFILE)) {
        ...
        if (retu < 0) {
            ...
            retu = avio_open(&output_format_context->pb, this->output_file_path.c_str(), AVIO_FLAG_WRITE);
            if(retu < 0) {
                spdlog::error("Failed to create output file: {}", this->output_file_path.c_str());
                return retu;
            }
        }
    }
...
int video::init_filter(FilteringContext *fctx, AVCodecContext *dec_ctx,
    ...
    if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
        ...
        if (!buffersrc || !buffersink) {
            spdlog::error("filtering source or sink element not found");
        ...
        retu = avfilter_graph_create_filter(&buffersink_ctx, buffersink, "out",
                                            nullptr, nullptr, filter_graph);
        if (retu < 0) {
            spdlog::error("Cannot create audio buffer sink");
            return retu;
        }
...
int video::flush_encoder(unsigned int stream_index) {
    ...
    while (true) {
        spdlog::trace("Flushing stream #{} encoder", stream_index);
...
int video::write_subtitle_file() {
    ...
    if (retu < 0) {
        spdlog::error("Cannot open output file: {}", av_err2str(retu));
        return retu;
    }

    retu = init_filters();
    if (retu < 0) {
        spdlog::error("Couldn't init filters");
        return retu;
    }
    ...
    while (true) {
        ...
        spdlog::trace("Demuxer gave frame of stream_index {}", stream_index);
    ...
    for (i = 0; i < input_format_context->nb_streams; i++) {
        ...
        if (retu < 0) {
            spdlog::error("Flushing filter failed");
            return end(retu);
        }
        ...
        if (retu < 0) {
            spdlog::error("Flushing encoder failed");
            return end(retu);
        }
    }
...
int video::read_subtitle_file() {
    if (!checked_header) {
        spdlog::error("Must call has_steg_file() first");
        return -234;
    }

    spdlog::debug("Reading from the file properly and outputing here: {}", this->subtitle_file->get_path().c_str());
    this->init_read();
    ...
    while (this->no_of_frames > 0 && av_read_frame(format, pkt) >= 0) {
        if (pkt->stream_index == video_stream) {
            ...
            while (ret >= 0) {
                ...
                spdlog::debug("pict type = {}", av_get_picture_type_char(picture->pict_type));
                if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
                    spdlog::error("Failed to receive frame from packet");
                    break;
                }
                if (ret >= 0) {
                    this->read_x = 0;
                    this->read_y = 0;
                    spdlog::debug("Finding a frame_header from frame: {}, {}", this->read_x, this->read_y);
                    frame_header *h = read_steg_header(picture);
                    if (h != nullptr) {
                        auto no_of_chars = (h->char_data / this->no_of_bits_in_char) / block_size;
                        spdlog::debug("No of chars: {}", no_of_chars);
                        for (; no_of_chars > 0; --no_of_chars) {
                            char c = this->read_char_from_frame(picture);
                            if (c == '\n') {
                                this->subtitle_file->write_line(character_vector);
                                spdlog::trace("No of chars: {}", no_of_chars);
                        ...
                        spdlog::debug("Finished with frame: {}, {}, {}", no_of_chars, this->read_x, this->read_y);
                        --this->no_of_frames;
                    }
...
bool video::has_steg_file() {
    this->init_read();
    spdlog::debug("Reset read positions to 0");
    this->read_x = 0;
    this->read_y = 0;
    ...
    while (av_read_frame(format, pkt) >= 0) {
        if (pkt->stream_index == video_stream) {
            ...
            while (ret >= 0) {
                ...
                spdlog::trace("pict type = {}", av_get_picture_type_char(picture->pict_type));
                if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
                    spdlog::debug("Failed to receive frame from packet: {}", av_err2str(ret));
                    break;
                }
                if (ret >= 0) {
                    spdlog::debug("Finding a frame_header from frame: {}, {}", this->read_x, this->read_y);
...
int video::init_read() {
    spdlog::debug("Allocating packet on startup");
    pkt = av_packet_alloc();
    ...
    spdlog::debug("Opening input '{}' in libav", this->input_file_path.generic_string());
    ret = avformat_open_input(&format, this->input_file_path.c_str(), nullptr, nullptr);

    if (ret != 0) {
        spdlog::error("Couldn't open video file: {} {}", input_file_path.generic_string(), av_err2str(ret));
        return -1;
    }

    spdlog::debug("Finding stream information about the file");
    ret = avformat_find_stream_info(format, nullptr);

    if (ret < 0) {
        spdlog::error("Wasn't able to generate stream information for file: {} ", input_file_path.generic_string());
        return -1;
    }
    ...
    if (video_stream == -1337) {
        spdlog::error("This file doesn't contain a video stream: ", input_file_path.generic_string());
        return -1;
    }

    spdlog::debug("Finding the decoder for the video file");
    codec = avcodec_find_decoder(format->streams[video_stream]->codecpar->codec_id);
    spdlog::debug("Initialising a parser with the codec: {}", codec->name);
    ...
    spdlog::debug("Allocating memory for the video codex context");
    ...
    spdlog::debug("Convert parameters to context");
    ...
    spdlog::debug("Open the input file: '{}'", input_file_path.generic_string());
    f = fopen(input_file_path.c_str(), "rb");

    if (!f) {
        spdlog::error("Could not open file: ", input_file_path.generic_string());
        return -1;
    }

    spdlog::debug("Allocate memory for the frames");
